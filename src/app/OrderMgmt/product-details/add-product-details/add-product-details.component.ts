...
@Component({
  ...
  providers: [ProductDetailsService, AppGlobals, DialogService, SharedService, HsnMasterService]
})
export class AddProductDetailsComponent implements OnInit {
  ...
  ProductDetailsEntityData = {
    ...
    "quantity": 0, "amount": 0, "discount": 0,
    ...
    "purchaseOrder": null, "isHistoricData": sessionStorage.getItem('isHistoricData'), "applyGstFlag": sessionStorage.getItem('applyGstFlag'), "currency": sessionStorage.getItem('currencyFlag'), "stateFlag": sessionStorage.getItem('sameStateFlag')
  };
  ...
  cancel = function () {
    ...
    if (this.route.snapshot.params.page == 'edit') {
      //this.router.navigate(['searchPurchaseOrder/SearchProductDetails/' + this.ProductDetailsEntityData.purchaseOrder.entityId + '/list']);
      if (this.route.snapshot.params.task == null) {
        this.router.navigate(['searchPurchaseOrder/SearchProductDetails', this.ProductDetailsEntityData.purchaseOrder.entityId, 'list']);
      } else {
        this.router.navigate(['searchTask/SearchProductDetails', this.ProductDetailsEntityData.purchaseOrder.entityId, 'list', this.route.snapshot.params.task]);
      }
  ...
  saveProductDetails = function () {
    ...
    this.PoParent.entityId = this.route.snapshot.params.id;
    this.ProductDetailsEntityData.purchaseOrder = this.PoParent;
  ...
  getProductList = function () {
    ...
    this.sharedService.getAllProductByOrg(sessionStorage.getItem("poOrgId"),headers).subscribe(resp => {
      debugger;
  ...
  //rate contract product List
  getRcProductList = function (rcId) {
    debugger;
    const headers = { "Authorization": sessionStorage.getItem("token") };
    this.showLoading = true;
    this.sharedService.getListOfProductsInRc(rcId, headers).subscribe(resp => {
      debugger;
      this.productList = resp;
      this.showLoading = false;
    }, (error: any) => {
  ...
  ngOnInit() {
    ...
    if (sessionStorage.getItem("rcId") == null) {
      this.getProductList();
    } else {
      this.getRcProductList(sessionStorage.getItem("rcId"));
    }
  }
